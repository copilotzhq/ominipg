...
try {
  ...
  console.log("✗ UUID extension not available:", (error as Error).message);
}
...
try {
  ...
  console.log("✗ Vector extension not available:", (error as Error).message);
...
console.log(
  `  UUID (uuid-ossp): ${uuidAvailable ? "✓ Working" : "✗ Not available"}`,
);
console.log(
  `  Vector (pgvector): ${vectorAvailable ? "✓ Working" : "✗ Not available"}`,
);

console.log("\n🎉 PGlite extensions demo completed!");
console.log(
  "💡 Tip: Extensions are only loaded when using PGlite, not PostgreSQL.",
);
console.log(
  "💡 This example gracefully handles cases where extensions are not available.",
);
console.log(
  "💡 Extension availability depends on the PGlite version and build configuration.",
);
