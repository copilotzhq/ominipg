...
await db.query('INSERT INTO users (name, email) VALUES ($1, $2)', ['Charlie', 'charlie@example.com']);
...
await db.query('UPDATE users SET email = $1 WHERE name = $2', ['alice.doe@example.com', 'Alice']);

const updatedUser = await db.query('SELECT * FROM users WHERE name = $1', ['Alice']);
