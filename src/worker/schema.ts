...
export async function createTableFromRemote(
  ...
  try {
    ...
    if (schemaResult.rows.length === 0) {
      throw new Error(
        `Table '${tableName}' not found in remote database's 'public' schema.`,
      );
    }
    ...
    const primaryKeys = (pkResult.rows as Array<{ column_name: string }>).map((
      r,
    ) => ident(r.column_name));

    // Build CREATE TABLE statement
    const columns = (schemaResult.rows as Array<
      {
        column_name: string;
        data_type: string;
        not_null: boolean;
    ...
    const createTableSQL = `CREATE TABLE ${ident(tableName)} (${
      columns.join(", ")
    })`;
  ...
  try {
    ...
    const result = await (client as unknown as {
      query: (
        ...
        { rows: Array<{ attname: string; indisprimary: boolean }> }
      >;
    }).query(query, [table]);
    ...
    const non = result.rows.filter((r) => !r.indisprimary).map((r) =>
      r.attname
    );
