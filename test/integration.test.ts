...
if (!SYNC_DB_URL) {
  Deno.test({ name: "E2E Sync Test", ignore: true, fn: () => {} });
    ...
    const cleaner = new (await import("npm:pg")).Pool({
    ...
    await t.step("Insert local data and push", async () => {
      ...
      const inserted = result.rows[0] as { id: number };
      localId = inserted.id;
    ...
    await t.step("Verify data on remote", async () => {
      const remote = new (await import("npm:pg")).Pool({
        connectionString: SYNC_DB_URL,
      });
    ...
    await t.step("Update data on remote and pull", async () => {
      ...
      await remote.query(
        `UPDATE ${TABLE} SET completed = true, updated_at = NOW() WHERE id = $1`,
        [localId],
      );
      await remote.end();
    ...
    await t.step("Verify change on local", async () => {
      ...
      assert(
        ...
        "Local record should be marked as completed",
      );
    });
...
if (!SYNC_DB_URL) {
  Deno.test({
    name: "Initial Sync from Remote to Local",
    ignore: true,
    fn: () => {},
  });
    ...
    await t.step("Verify pre-existing data is synced locally", async () => {
      ...
      assertEquals(
        rows.length,
        1,
        "Should have synced one pre-existing record.",
      );
...
if (!SYNC_DB_URL) {
  Deno.test({
    ...
    await t.step("Verify data synced to remote", async () => {
      const remoteVerifier = new (await import("npm:pg")).Pool({
        connectionString: SYNC_DB_URL,
      });
      try {
        const { rows } = await remoteVerifier.query(
          `SELECT * FROM ${TABLE} WHERE id = $1`,
          [localId],
        );
        assertEquals(
          rows.length,
          1,
          "Record should exist on remote after push.",
        );
