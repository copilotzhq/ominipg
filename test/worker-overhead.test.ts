...
Deno.test({
  name: "Measure Deno Worker overhead",
  ...
  const worker = new Worker(
    new URL("./_simple_worker.ts", import.meta.url).href,
    { type: "module" },
  );
  ...
  if (before != null && afterCreate != null) {
    console.log(
      `Worker created (+${afterCreate - before} MB, rss=${afterCreate} MB)`,
    );
  } else {
  ...
  if (afterCreate != null && afterClose != null) {
    console.log(
      `After worker closed (delta ${
        afterClose - afterCreate
      } MB, rss=${afterClose} MB)`,
    );
  }
